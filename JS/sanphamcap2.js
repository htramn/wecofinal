...
//load sản phẩm lên giao diện
 function loadDoc(ID_CATEGORY) {
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        myFunction(this,ID_CATEGORY);
        }      
    };
    xhttp.open("GET", "XML//products.xml", true);
    xhttp.send();
  }
  function myFunction(xml,ID_CATEGORY) {
    var i;
    ...
    for (i = 0; i <dsSanPham.length; i++) {
      var sanpham = dsSanPham[i].getElementsByTagName("PRODUCT");
        ...
        {
            if(sanpham[j].getElementsByTagName("ID_CATEGORY")[0].childNodes[0].nodeValue==ID_CATEGORY)
            {
